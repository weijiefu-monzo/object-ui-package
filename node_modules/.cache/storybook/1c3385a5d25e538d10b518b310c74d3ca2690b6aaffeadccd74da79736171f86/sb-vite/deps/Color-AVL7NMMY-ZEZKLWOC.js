...
var require_conversions = __commonJS({ "../../node_modules/color-convert/conversions.js"(exports, module) {
  ...
  convert2.rgb.cmyk = function(rgb) {
    let r2 = rgb[0] / 255, g2 = rgb[1] / 255, b2 = rgb[2] / 255, k2 = Math.min(1 - r2, 1 - g2, 1 - b2), c2 = (1 - r2 - k2) / (1 - k2) || 0, m2 = (1 - g2 - k2) / (1 - k2) || 0, y22 = (1 - b2 - k2) / (1 - k2) || 0;
    return [c2 * 100, m2 * 100, y22 * 100, k2 * 100];
  };
  function comparativeDistance(x2, y22) {
    return (x2[0] - y22[0]) ** 2 + (x2[1] - y22[1]) ** 2 + (x2[2] - y22[2]) ** 2;
  }
  ...
  convert2.rgb.xyz = function(rgb) {
    ...
    let x2 = r2 * 0.4124 + g2 * 0.3576 + b2 * 0.1805, y22 = r2 * 0.2126 + g2 * 0.7152 + b2 * 0.0722, z2 = r2 * 0.0193 + g2 * 0.1192 + b2 * 0.9505;
    return [x2 * 100, y22 * 100, z2 * 100];
  };
  convert2.rgb.lab = function(rgb) {
    let xyz = convert2.rgb.xyz(rgb), x2 = xyz[0], y22 = xyz[1], z2 = xyz[2];
    x2 /= 95.047, y22 /= 100, z2 /= 108.883, x2 = x2 > 8856e-6 ? x2 ** (1 / 3) : 7.787 * x2 + 16 / 116, y22 = y22 > 8856e-6 ? y22 ** (1 / 3) : 7.787 * y22 + 16 / 116, z2 = z2 > 8856e-6 ? z2 ** (1 / 3) : 7.787 * z2 + 16 / 116;
    let l2 = 116 * y22 - 16, a2 = 500 * (x2 - y22), b2 = 200 * (y22 - z2);
    return [l2, a2, b2];
  };
  ...
  convert2.hsv.rgb = function(hsv) {
    let h2 = hsv[0] / 60, s2 = hsv[1] / 100, v2 = hsv[2] / 100, hi = Math.floor(h2) % 6, f2 = h2 - Math.floor(h2), p2 = 255 * v2 * (1 - s2), q22 = 255 * v2 * (1 - s2 * f2), t2 = 255 * v2 * (1 - s2 * (1 - f2));
    switch (v2 *= 255, hi) {
      case 0:
        return [v2, t2, p2];
      case 1:
  ...
  convert2.cmyk.rgb = function(cmyk) {
    let c2 = cmyk[0] / 100, m2 = cmyk[1] / 100, y22 = cmyk[2] / 100, k2 = cmyk[3] / 100, r2 = 1 - Math.min(1, c2 * (1 - k2) + k2), g2 = 1 - Math.min(1, m2 * (1 - k2) + k2), b2 = 1 - Math.min(1, y22 * (1 - k2) + k2);
    return [r2 * 255, g2 * 255, b2 * 255];
  };
  convert2.xyz.rgb = function(xyz) {
    let x2 = xyz[0] / 100, y22 = xyz[1] / 100, z2 = xyz[2] / 100, r2, g2, b2;
    return r2 = x2 * 3.2406 + y22 * -1.5372 + z2 * -0.4986, g2 = x2 * -0.9689 + y22 * 1.8758 + z2 * 0.0415, b2 = x2 * 0.0557 + y22 * -0.204 + z2 * 1.057, r2 = r2 > 31308e-7 ? 1.055 * r2 ** (1 / 2.4) - 0.055 : r2 * 12.92, g2 = g2 > 31308e-7 ? 1.055 * g2 ** (1 / 2.4) - 0.055 : g2 * 12.92, b2 = b2 > 31308e-7 ? 1.055 * b2 ** (1 / 2.4) - 0.055 : b2 * 12.92, r2 = Math.min(Math.max(0, r2), 1), g2 = Math.min(Math.max(0, g2), 1), b2 = Math.min(Math.max(0, b2), 1), [r2 * 255, g2 * 255, b2 * 255];
  };
  convert2.xyz.lab = function(xyz) {
    let x2 = xyz[0], y22 = xyz[1], z2 = xyz[2];
    x2 /= 95.047, y22 /= 100, z2 /= 108.883, x2 = x2 > 8856e-6 ? x2 ** (1 / 3) : 7.787 * x2 + 16 / 116, y22 = y22 > 8856e-6 ? y22 ** (1 / 3) : 7.787 * y22 + 16 / 116, z2 = z2 > 8856e-6 ? z2 ** (1 / 3) : 7.787 * z2 + 16 / 116;
    let l2 = 116 * y22 - 16, a2 = 500 * (x2 - y22), b2 = 200 * (y22 - z2);
    return [l2, a2, b2];
  };
  convert2.lab.xyz = function(lab) {
    let l2 = lab[0], a2 = lab[1], b2 = lab[2], x2, y22, z2;
    y22 = (l2 + 16) / 116, x2 = a2 / 500 + y22, z2 = y22 - b2 / 200;
    let y222 = y22 ** 3, x22 = x2 ** 3, z22 = z2 ** 3;
    return y22 = y222 > 8856e-6 ? y222 : (y22 - 16 / 116) / 7.787, x2 = x22 > 8856e-6 ? x22 : (x2 - 16 / 116) / 7.787, z2 = z22 > 8856e-6 ? z22 : (z2 - 16 / 116) / 7.787, x2 *= 95.047, y22 *= 100, z2 *= 108.883, [x2, y22, z2];
  };
...
var m = Rn.memo(function(o2) {
  var a2 = o2.onMove, l2 = o2.onKey, s2 = c(o2, ["onMove", "onKey"]), m2 = A(null), g2 = i(a2), p2 = i(l2), b2 = A(null), _22 = A(false), x2 = T(function() {
    var e2 = function(e3) {
      h(e3), (f(e3) ? e3.touches.length > 0 : e3.buttons > 0) && m2.current ? g2(d2(m2.current, e3, b2.current)) : t2(false);
    }, r2 = function() {
      return t2(false);
    };
    function t2(t3) {
      var n2 = _22.current, o3 = v(m2.current), a3 = t3 ? o3.addEventListener : o3.removeEventListener;
    ...
    return [function(e3) {
      ...
      if (n2 && (h(r3), !(function(e4, r4) {
        ...
        n2.focus(), g2(d2(n2, r3, b2.current)), t2(true);
      }
    }, function(e3) {
  ...
  return y(function() {
    return H2;
  }, [H2]), Rn.createElement("div", u({}, s2, { onTouchStart: C2, onMouseDown: C2, className: "react-colorful__interactive", ref: m2, onKeyDown: E2, tabIndex: 0, role: "slider" }));
});
var g = function(e2) {
...
var p = function(r2) {
  ...
  return Rn.createElement("div", { className: l2, style: { top: 100 * a2 + "%", left: 100 * n2 + "%" } }, Rn.createElement("div", { className: "react-colorful__pointer-fill", style: { backgroundColor: t2 } }));
};
...
var E = function(e2, r2) {
  return r2 === void 0 && (r2 = "deg"), Number(e2) * (_2[r2] || 1);
};
var H = function(e2) {
...
var S = Rn.memo(function(r2) {
  var t2 = r2.hue, n2 = r2.onChange, o2 = g(["react-colorful__hue", r2.className]);
  return Rn.createElement("div", { className: o2 }, Rn.createElement(m, { onMove: function(e2) {
    n2({ h: 360 * e2.left });
  }, onKey: function(e2) {
    n2({ h: s(t2 + 360 * e2.left, 0, 360) });
  }, "aria-label": "Hue", "aria-valuenow": b(t2), "aria-valuemax": "360", "aria-valuemin": "0" }, Rn.createElement(p, { className: "react-colorful__hue-pointer", left: t2 / 360, color: q2({ h: t2, s: 100, v: 100, a: 1 }) })));
});
var T2 = Rn.memo(function(r2) {
  var t2 = r2.hsva, n2 = r2.onChange, o2 = { backgroundColor: q2({ h: t2.h, s: 100, v: 100, a: 1 }) };
  return Rn.createElement("div", { className: "react-colorful__saturation", style: o2 }, Rn.createElement(m, { onMove: function(e2) {
    n2({ s: 100 * e2.left, v: 100 - 100 * e2.top });
  }, onKey: function(e2) {
    n2({ s: s(t2.s + 100 * e2.left, 0, 100), v: s(t2.v - 100 * e2.top, 0, 100) });
  }, "aria-label": "Color", "aria-valuetext": "Saturation " + b(t2.s) + "%, Brightness " + b(t2.v) + "%" }, Rn.createElement(p, { className: "react-colorful__saturation-pointer", top: 1 - t2.v / 100, left: t2.s / 100, color: q2(t2) })));
});
...
function Y(e2, t2, l2) {
  var u2 = i(l2), c2 = d(function() {
    return e2.toHsva(t2);
  }), s2 = c2[0], f2 = c2[1], v2 = A({ color: t2, hsva: s2 });
  y(function() {
...
var U = function(t2) {
  var n2 = t2.className, o2 = t2.colorModel, a2 = t2.color, l2 = a2 === void 0 ? o2.defaultColor : a2, i2 = t2.onChange, s2 = c(t2, ["className", "colorModel", "color", "onChange"]), f2 = A(null);
  Q(f2);
  var v2 = Y(o2, l2, i2), d22 = v2[0], h2 = v2[1], m2 = g(["react-colorful", n2]);
  return Rn.createElement("div", u({}, s2, { ref: f2, className: m2 }), Rn.createElement(T2, { hsva: d22, onChange: h2 }), Rn.createElement(S, { hue: d22.h, onChange: h2, className: "react-colorful__last-control" }));
};
var W = { defaultColor: "000", toHsva: x, fromHsva: function(e2) {
  return w({ h: e2.h, s: e2.s, v: e2.v, a: 1 });
}, equal: X };
var Z = function(r2) {
  return Rn.createElement(U, u({}, r2, { colorModel: W }));
};
var ee = function(r2) {
  ...
  return Rn.createElement("div", { className: l2 }, Rn.createElement("div", { className: "react-colorful__alpha-gradient", style: a2 }), Rn.createElement(m, { onMove: function(e2) {
    o2({ a: e2.left });
  }, onKey: function(e2) {
    o2({ a: s(n2.a + e2.left) });
  }, "aria-label": "Alpha", "aria-valuetext": u2 + "%", "aria-valuenow": u2, "aria-valuemin": "0", "aria-valuemax": "100" }, Rn.createElement(p, { className: "react-colorful__alpha-pointer", left: n2.a, color: k(n2) })));
};
var re = function(t2) {
  var n2 = t2.className, o2 = t2.colorModel, a2 = t2.color, l2 = a2 === void 0 ? o2.defaultColor : a2, i2 = t2.onChange, s2 = c(t2, ["className", "colorModel", "color", "onChange"]), f2 = A(null);
  Q(f2);
  var v2 = Y(o2, l2, i2), d22 = v2[0], h2 = v2[1], m2 = g(["react-colorful", n2]);
  return Rn.createElement("div", u({}, s2, { ref: f2, className: m2 }), Rn.createElement(T2, { hsva: d22, onChange: h2 }), Rn.createElement(S, { hue: d22.h, onChange: h2 }), Rn.createElement(ee, { hsva: d22, onChange: h2, className: "react-colorful__last-control" }));
};
var le = { defaultColor: "hsla(0, 0%, 0%, 1)", toHsva: H, fromHsva: k, equal: P };
var ue = function(r2) {
  return Rn.createElement(re, u({}, r2, { colorModel: le }));
};
...
var He = function(r2) {
  return Rn.createElement(re, u({}, r2, { colorModel: Ee }));
};
...
var Swatch = ({ value, style, ...props }) => {
  ...
  return Rn.createElement(SwatchColor, { ...props, style: { ...style, backgroundImage } });
};
...
var stringToArgs = (value) => {
  ...
  let [, x2, y22, z2, a2 = 1] = match;
  return [x2, y22, z2, a2].map(Number);
};
var parseRgb = (value) => {
...
var useColorInput = (initialValue, onChange) => {
  let [value, setValue] = d(initialValue || ""), [color, setColor] = d(() => parseValue(value)), [colorSpace, setColorSpace] = d(color?.colorSpace || "hex");
  y(() => {
  ...
  let realValue = T(() => getRealValue(value, color, colorSpace).toLowerCase(), [value, color, colorSpace]), updateValue = q((update) => {
...
var usePresets = (presetColors, currentColor, colorSpace) => {
  let [selectedColors, setSelectedColors] = d(currentColor?.valid ? [currentColor] : []);
  y(() => {
    currentColor === void 0 && setSelectedColors([]);
  }, [currentColor]);
  let presets = T(() => (presetColors || []).map((preset) => typeof preset == "string" ? parseValue(preset) : preset.title ? { ...parseValue(preset.color), keyword: preset.title } : parseValue(preset.color)).concat(selectedColors).filter(Boolean).slice(-27), [presetColors, selectedColors]), addPreset = q((color) => {
...
var ColorControl = ({ name, value: initialValue, onChange, onFocus, onBlur, presetColors, startOpen = false, argType }) => {
  let debouncedOnChange = q(debounce2(onChange, 200), [onChange]), { value, realValue, updateValue, color, colorSpace, cycleColorSpace } = useColorInput(initialValue, debouncedOnChange), { presets, addPreset } = usePresets(presetColors ?? [], color, colorSpace), Picker = ColorPicker[colorSpace], readonly = !!argType?.table?.readonly;
  return Rn.createElement(Wrapper, { "aria-readonly": readonly }, Rn.createElement(PickerTooltip, { startOpen, trigger: readonly ? null : void 0, closeOnOutsideClick: true, onVisibleChange: () => color && addPreset(color), tooltip: Rn.createElement(TooltipContent, null, Rn.createElement(Picker, { color: realValue === "transparent" ? "#000000" : realValue, onChange: updateValue, onFocus, onBlur }), presets.length > 0 && Rn.createElement(Swatches, null, presets.map((preset, index) => Rn.createElement(Y7, { key: `${preset?.value || index}-${index}`, hasChrome: false, tooltip: Rn.createElement(Note, { note: preset?.keyword || preset?.value || "" }) }, Rn.createElement(Swatch, { value: preset?.[colorSpace] || "", active: !!(color && preset && preset[colorSpace] && id(preset[colorSpace] || "") === id(color[colorSpace])), onClick: () => preset && updateValue(preset.value || "") }))))) }, Rn.createElement(Swatch, { value: realValue, style: { margin: 4 } })), Rn.createElement(Input, { id: getControlId(name), value, onChange: (e2) => updateValue(e2.target.value), onFocus: (e2) => e2.target.select(), readOnly: readonly, placeholder: "Choose color..." }), value ? Rn.createElement(ToggleIcon, { onClick: cycleColorSpace }) : null);
};
...
//# sourceMappingURL=Color-AVL7NMMY-ZEZKLWOC.js.map
